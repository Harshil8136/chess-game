...
| Property | Value |
|----------|-------|
| **Project ID** | `pklzjomfwqzricpsyrrc` |
| **Project Name** | cf-astro |
| **Region** | us-east-1 |
| **Database Host** | `db.pklzjomfwqzricpsyrrc.supabase.co` |
| **REST API** | `https://pklzjomfwqzricpsyrrc.supabase.co/rest/v1/` |
...
### 6.13 Cloudflare Queues — Async Email Architecture (Active)

> **Status:** ✅ Implemented and deployed (March 2026)

#### Overview

All transactional emails are delivered **asynchronously** via a Cloudflare Queue + sidecar consumer worker. No API route in the main Astro project calls Resend directly. This decouples email delivery from the user-facing request lifecycle — the booking/ARCO response returns instantly after DB insert.
...
│                                                                 │
│  /api/booking.ts ──→ DB insert ──→ env.EMAIL_QUEUE.send({...})  │
│  /api/arco/submit.ts ──→ DB + R2 ──→ env.EMAIL_QUEUE.send({...}) │
│                                                                 │
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│  SIDECAR WORKER: cf-astro-email-consumer (queue-worker/)        │
│                                                                 │
...
| `queue-worker/src/index.ts` | **Consumer** — builds HTML + calls Resend | Changing templates, adding types |
| `queue-worker/wrangler.toml` | Consumer config (batch size, retries, DLQ) | Changing retry/batch behavior |
...
- `connect-src 'self' *.posthog.com *.ingest.us.sentry.io *.supabase.co api.resend.com`
...
- Build + `astro check` must pass before merge
