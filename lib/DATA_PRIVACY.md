...
## Access Control
Following the PLAC (Page Level Access Control) architecture, this module is highly restricted.
- **Minimum Role**: `SUPER_ADMIN`
- **Location**: `/dashboard/privacy`
- **Sidebar Integration**: Displayed automatically when permitted via PLAC mappings.
- **API Defense**: Deep validation checks ensuring strict access control using `requireAuth(context, ROLES.SUPER_ADMIN)` at the Cloudflare Pages Function layer (`api/privacy/logs`).
