...
- Use Cloudflare Workers Free plan plus existing services (D1, KV, R2, Upstash Redis, Supabase) for state, RAG, and configuration.
- Integrate directly with WhatsApp Cloud API to avoid third-party BSP markups, acknowledging that Metaâ€™s per-message pricing is unavoidable but can be minimized by structuring messaging as service conversations.[^2][^3][^4]
- Provide a unified conversation engine for both web and WhatsApp with centralized security, audit logging, and admin-controlled behavior.
...
Each LLM model has different neuron-per-token characteristics, but small and mid-size models (for example, 1â€“8B parameters) remain inexpensive at typical chat token lengths for SMB use.
...
Cloudflare Vectorize pricing is based on â€œstored dimensionsâ€ (vector count Ã— dimension) and â€œqueried dimensions.â€[^8]
...
This free tier is enough for a significant knowledge base (tens of thousands of short documents with 384â€“1,024-dimensional embeddings) and tens of thousands of queries per month.

Cloudflareâ€™s documentation and community answers emphasize that Vectorize remains available within the Workers Free tier for prototyping and light production workloads, though high-volume use benefits from Workers Paid.[^10][^11][^8]
...
Additionally, the first 1,000 customer-initiated conversations per month are typically free for Cloud API accounts, after which Metaâ€™s per-message fees apply.[^12]

Third-party Business Solution Providers (BSPs) add their own per-message or per-conversation surcharges on top of Metaâ€™s base rates.[^2][^12]
...
3. **Retrieve conversation context**
   - Load last N messages from D1 or Supabase for that session.
   - Truncate or summarize older messages to stay within LLM context limits.
4. **RAG: retrieve relevant knowledge**
   - Embed the user query with Workers AI embedding model such as `@cf/baai/bge-small-en-v1.5`, which has low cost and good English performance.[^1]
...
6. **Call Workers AI**
   - Use a cost-effective chat model (for example, a 3â€“8B parameter Llama or Granite model) to balance cost and quality; Cloudflareâ€™s per-token costs for these models remain low at SMB scale.[^5][^1]
...
8. **Persist logs**
   - Write message, model call metadata (model name, tokens, latency, cost estimate), and RAG document IDs into D1 or Supabase.
   - Store a summarized/hashed representation for long-term analytics while keeping full text only as long as necessary for debugging and audits.
9. **Return response**
...
WhatsApp Cloud API delivers incoming messages via HTTPS POST webhooks containing the senderâ€™s phone number, message body, and metadata.[^12]
...
1. **Verify request authenticity**
   - Validate Metaâ€™s signature header using the app secret to prevent spoofing.
...
5. **Send reply via WhatsApp Cloud API**
   - Use WhatsAppâ€™s `messages` endpoint with type `text` or `interactive` for structured replies, respecting free service window rules to minimize billing.[^4][^2][^3]
   - Never send marketing templates automatically; require explicit admin action if ever needed.
6. **Log and audit**
   - Persist inbound and outbound messages, including WhatsApp IDs, to D1 or Supabase.
...
- Avoid outbound marketing templates from the chatbot; reserve such flows for manual campaigns where the incremental value justifies cost.
...
- **R2**: markdown, HTML, or JSON exports of content for embedding (e.g., nightly job generates `kb/*.md`).
...
- Choose an embedding model like `@cf/baai/bge-small-en-v1.5` or `bge-base-en-v1.5` for good quality at low cost and 384â€“768 dimensions.[^1]
- Use a schema where each vector record includes:
  - `id` (stable key referencing your CMS entry),
  - `type` (e.g., `policy`, `amenity`, `booking-faq`),
  - `language`,
  - `source_url` or `cms_path`,
...
- Query by:
  ...
  - Returning top K = 3â€“8 documents.
...
Given cf-adminâ€™s RBAC layers (DEV â†’ SuperAdmin â†’ Admin â†’ Staff) and strong audit logging, it is a natural control plane for the chatbot.
Recommended configuration tables in Supabase:

- **`chatbot_channels`**
  - `id`, `name` (`web`, `whatsapp`), `enabled`.
  - `llm_model` (Workers AI model identifier).
  - `temperature`, `max_tokens`, `top_p`.
  - `allow_rag` (boolean), `rag_top_k`, `rag_sources`.
- **`chatbot_prompts`**
...
- Key rotation and Webhook secret management (only DEV / SuperAdmin).
...
- Log every inbound and outbound chatbot message into a dedicated table with:
  ...
  - `rag_documents` (IDs of documents used).
  - `llm_model`, estimated `tokens_in`, `tokens_out`, and approximate cost.
- Use Cloudflare AI Gateway logs for low-level telemetry like response time, errors, and model-level usage limits.[^7]
- Periodically summarize logs to maintain performance and control storage; for example, keep raw text for 30â€“90 days and longer-term aggregated metrics.
...
- Restrict WhatsApp webhooks to Meta IP ranges via Cloudflare WAF rules and validate signatures.[^12]
- For web chat, apply origin checks and CSRF-style protections on POST endpoints.
- Use route patterns and service bindings to ensure internal configuration APIs are reachable only from cf-admin and not public.

### Data Protection

- Avoid storing sensitive personal data from WhatsApp messages whenever possible; implement keyword or entity detection to mask fields like emails or phone numbers in logs.
- Use Supabase RLS policies so that only authorized roles can query chat logs or configuration.
...
- Implement a global â€œkill switchâ€ in cf-admin to disable the chatbot on a channel instantly in case of incidents.
...
- **Workers AI**: 10,000 neurons/day free; careful prompt design and smaller models keep token usage low, and costs beyond free are predictable at 0.011 USD per 1,000 neurons.[^1]
- **Workers**: 100,000 requests/day on the Free tier is sufficient for moderate chat volume; upgrade to Paid at 5 USD/month if needed.[^6]
- **Vectorize**: Free tier supports millions of stored and queried dimensions, enough for a substantial knowledge base and hundreds to thousands of queries per day.[^8][^9]
- **AI Gateway**: No extra per-call fee; free features include logging, analytics, caching, and rate limiting, within its log quota.[^7]
- **Supabase, D1, KV, Upstash**: All have generous free tiers that suit a single-business deployment when used efficiently with summarized logs and limited retention.
- **WhatsApp Cloud API**: Core cost driver outside Cloudflare; by relying mostly on user-initiated service conversations within the 24-hour window and avoiding marketing templates, effective messaging costs can be driven close to zero for small volumes.[^3][^4][^2]

## Implementation Roadmap

A practical implementation plan could run in phases:
...
3. **Admin Integration**
   - Add configuration tables in Supabase and configuration UI in cf-admin.
...
5. **Hardening and Optimization**
   - Add rate limiting, WAF rules, redaction, and detailed audit logging.
...
1. [Pricing Â· Cloudflare Workers AI docs](https://developers.cloudflare.com/workers-ai/platform/pricing/) - Workers AI is included in both the Free and Paid Workers plans and is priced at $0.011 per 1,000 Neu...

2. [WhatsApp Business API Pricing 2026: Complete Guide - Flowcall](https://flowcall.co/blog/whatsapp-business-api-pricing-2026) - WhatsApp Business API pricing guide 2026: per-message costs by country, free messaging windows, volu...

3. [WhatsApp Business API Pricing](https://help.sleekflow.io/en_US/whatsapp/pricing) - Per-message pricing Starting 1 July 2025 at 12:00 AM (based on your WhatsApp Business Accountâ€™s time...

4. [WhatsApp API Pricing Update: Effective July 1, 2025](https://www.ycloud.com/blog/whatsapp-api-pricing-update) - Everything you need to know about Meta's latest WhatsApp API pricing updates, effective 1st July 202...

5. [Pricing - Workers AI - Cloudflare Docs](https://cloudflare-docs-7ou.pages.dev/workers-ai/platform/pricing/) - Workers AI is included in both the Free and Paid Workers plans and is priced based on model task, mo...

6. [Cloudflare Workers Pricing Plans (2026) - CompareTiers](https://comparetiers.com/tools/cloudflare-workers) - Serverless execution on Cloudflare's edge network. Compare Cloudflare Workers pricing: 4 plans, from...

7. [Cloudflare AI Gateway Pricing Explained For 2026](https://www.truefoundry.com/blog/cloudflare-ai-gateway-pricing-a-complete-breakdown) - Learn how Cloudflare AI Gateway pricing works, what impacts cost, and when teams consider alternativ...

8. [Pricing Â· Cloudflare Vectorize docs](https://developers.cloudflare.com/vectorize/platform/pricing/) - Vectorize bills are based on:

9. [Cloudflare Vectorize Cost Calculator - LLMBase](https://llmbase.ai/tools/cloudflare-cost-calculator/) - Free Cloudflare Vectorize cost calculator. Estimate monthly costs for vector storage and queries. Ca...

10. [cloudflare-docs/src/content/docs/vectorize/platform/pricing.mdx at production Â· cloudflare/cloudflare-docs](https://github.com/cloudflare/cloudflare-docs/blob/production/src/content/docs/vectorize/platform/pricing.mdx) - Cloudflareâ€™s documentation. Contribute to cloudflare/cloudflare-docs development by creating an acco...

11. [Demystifying Cloudflare Vectorize Pricing: What You Need ...](https://www.oreateai.com/blog/demystifying-cloudflare-vectorize-pricing-what-you-need-to-know/652bcbf63978567606441bea3e39f217) - Understand Cloudflare Vectorize pricing, which is integrated with Cloudflare Workers. Learn about th...

12. [WhatsApp Cloud API: Setup & Cost Guide (2026) | Chatarmin](https://chatarmin.com/en/blog/whatsapp-cloudapi) - Cloud API or On-Premise? Everything about Meta's WhatsApp Cloud API. Setup guide, hosting benefits, ...
