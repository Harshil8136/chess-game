...
Last Updated: 2026-04-03 Research Sources: Cloudflare Docs MCP, Supabase MCP, Cloudflare Bindings MCP, Tavily, Official Documentation
...
Storage	Cloudflare R2 (admin uploads — future phase)
...
Authorization Model
Supabase signup is DISABLED in dashboard settings
Only users listed in admin_authorized_users table can access the portal
SuperAdmin/DEV adds users to the whitelist with assigned roles
GoTrue issues JWTs for valid auth attempts; application layer checks whitelist
Hardcoded super admin emails serve as ultimate failsafe
Session Security
...
Shield Icon — w-12 h-12 rounded-xl, violet-tinted bg
Heading — text-[22px] font-bold text-white tracking-tight
Subtext — text-[13px] text-white/45 font-medium
Social Buttons — Full-width stacked (Google first, then GitHub), py-3 rounded-xl, gap-3
Divider — Gradient lines with OR label (text-[11px] text-white/30 uppercase tracking-widest)
Email Input — px-4 py-3 rounded-xl, bg-rgba(255,255,255,0.03), violet focus ring
CTA Button — py-3 rounded-xl font-semibold, disabled=transparent violet / enabled=white-to-lavender gradient w/ glow
...
Global CSS lives in src/styles/global.css — imported by AuthLayout.astro
Tailwind v4 uses @theme block for design tokens and @source directives for component scanning
⚠️ CRITICAL: ALL base resets/styles MUST be inside @layer base { }. Unlayered CSS overrides Tailwind utilities and causes layout collapse (padding, margin, flex all break). This was the root cause of the original "squishy card" bug
Animation keyframes (authFadeIn, orbDrift1-3) are scoped inside AuthLayout.astro's <style> block, NOT in global.css
IDE lint warnings for @source, @theme are false positives — Tailwind v4 syntax not yet recognized by CSS linters
File Map
File	Purpose
...
13.3 Perplexity MCP — PAID SERVICE
@mcp:perplexity-ask costs real money. Use ONLY as last resort after exhausting all free tools.

Priority Order:

RULES.md → 2. SKILL.md files → 3. @mcp:cloudflare-docs → 4. @mcp:tavily → 5. Pre-trained knowledge → 6. @mcp:perplexity-ask (💰 LAST)
14. TOTAL MONTHLY COST — $0
Service	What We Use	Monthly Cost
Cloudflare Workers	Hosting + SSR	$0
Cloudflare KV	Session storage	$0
Cloudflare D1	Operational data	$0
