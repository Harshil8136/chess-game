...
// ADDED: A robust helper function to check if a square is attacked.
/**
...
function isSquareAttacked(square, attackerColor, gameInstance) {
    // Create a temporary game state to avoid altering the main one.
    const tempGame = new Chess(gameInstance.fen());
    const piece = tempGame.get(square);

    // If the square contains a king of the attacker's color, we can't place another one to test.
    if (piece && piece.type === 'k' && piece.color === attackerColor) {
        return false;
    }

    // Remove any piece that's on the square.
    tempGame.remove(square);

    // Place a king of the *opposite* color of the attacker on the square.
    const kingColor = attackerColor === 'w' ? 'b' : 'w';
    if (!tempGame.put({ type: 'k', color: kingColor }, square)) {
        return false; // Should not happen in valid positions
    }
    
    // Temporarily set the turn to the attacker's side to use in_check().
    const fenParts = tempGame.fen().split(' ');
    fenParts[1] = attackerColor;
    tempGame.load(fenParts.join(' '));

    // If the newly placed king is in check, it means the square is attacked.
...
// UPDATED: This function now uses the robust isSquareAttacked helper for accurate threat detection.
function updateThreatHighlights() {
    ...
    game.SQUARES.forEach(square => {
        ...
        // Only show threats against the pieces of the player whose turn it is.
        if (piece && piece.color === threatenedPlayer) {
            // Use the new helper function to check if the square is attacked by the opponent.
