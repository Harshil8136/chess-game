...
function initApp() {
    ...
    $('.tab-button').on('click', function() { 
        Logger.info(`Tab changed to: ${$(this).data('tab')}`);
        showTab($(this).data('tab')); 
    });

    // UPDATED: The "New Game" button now respects the "Ask on New Game" setting.
    ...
    historyFirstBtn.on('click', () => { Logger.info('History First button clicked.'); if (!historyFirstBtn.prop('disabled')) { reviewMoveIndex = 0; showHistoryPosition(); } });
    historyPrevBtn.on('click', () => { Logger.info('History Previous button clicked.'); if (!historyPrevBtn.prop('disabled')) { if (reviewMoveIndex === null) reviewMoveIndex = game.history().length - 1; if (reviewMoveIndex > 0) reviewMoveIndex--; else reviewMoveIndex = 0; showHistoryPosition(); } });
    historyNextBtn.on('click', () => { Logger.info('History Next button clicked.'); if (!historyNextBtn.prop('disabled')) { if (reviewMoveIndex === null) return; if (reviewMoveIndex < game.history().length - 1) reviewMoveIndex++; showHistoryPosition(); } });
    historyLastBtn.on('click', () => { Logger.info('History Last button clicked.'); exitReviewMode(); });
    moveHistoryLog.on('click', '.move-span', function() { 
        reviewMoveIndex = parseInt($(this).data('move-index'));
        Logger.info(`Jumped to move ${reviewMoveIndex + 1} from history.`);
        showHistoryPosition(); 
    });
    ...
    hintButton.on('click', function() {
        ...
        if ($(this).prop('disabled') || !liveAnalysisStockfish) {
            Logger.warn('Hint request ignored: button disabled or analysis engine not ready.');
            return;
        }
    ...
    loadFenBtn.on('click', () => {
        ...
        Logger.info(`Load FEN button clicked.`, { fen: fen });
    ...
    exportPgnBtn.on('click', function() {
        Logger.info('Export PGN button clicked.');
         if ($(this).prop('disabled')) return;
         const pgn = game.pgn();
    ...
    threatsToggle.on('change', function() {
        ...
        Logger.info(`Highlight Threats toggled: ${highlightThreats}`);
    ...
    focusModeToggle.on('click', function() {
        ...
        Logger.info(`Focus Mode toggled: ${isFocus}`);
    ...
    // ADDED: Logic to save the "Ask on New Game" preference.
    showTimeControlModalToggle.on('change', function() {
        showModalOnRestart = $(this).is(':checked');
        Logger.info(`"Ask on New Game" preference set to: ${showModalOnRestart}`);
    ...
    soundToggle.on('click', () => { isMuted = !isMuted; localStorage.setItem('chessSoundMuted', isMuted); soundIcon.attr('src', isMuted ? 'icon/speaker-x-mark.png' : 'icon/speaker-wave.png'); Logger.info(`Sound toggled: ${!isMuted}`); });
    ...
    uiThemeSelector.on('change', () => {
        ...
        Logger.info(`UI theme changed to: ${selectedTheme}`);
    ...
    Promise.all([
        ...
        stockfish.onmessage = event => {
            ...
            if (message.startsWith('bestmove')) {
                const move = message.split(' ')[1];
                Logger.engine(`Best move: ${move}`, { eval: (lastEval / 100).toFixed(2) });
                performMove(move);
        ...
        // ADDED: Load the "Ask on New Game" preference.
        const savedShowModal = localStorage.getItem('chessShowModalOnRestart');
        showModalOnRestart = savedShowModal === null ? true : (savedShowModal === 'true');
        showTimeControlModalToggle.prop('checked', showModalOnRestart);

        // Once engines and settings are loaded, initialize the first game.
        restartButton.click();

    }).catch(error => {
        Logger.error('CRITICAL: Failed to load Stockfish engine.', error);
